...
@interface CVTaskEditViewController () <UIActionSheetDelegate, IPadDatePickerDelegate, CVDescViewDelegate, ContactPickerDelegate, CVFilePickerDelegate, UIAlertViewDelegate>
...
@property(nonatomic, retain) CVLabelCell* assigneesCell;
@property(nonatomic, retain) CVLabelCell* ccersCell;
...
@property(nonatomic, retain) NSArray* addedAssignees;
@property(nonatomic, retain) NSArray* addedCCers;
...
@property(nonatomic, retain) UIView* imageCollectionViewContainerForCCers;
@property(nonatomic, retain) UICollectionView* imageCollectionViewForCCers;

@property(nonatomic, retain) CVUserListItem* collectionContactItem;
@property(nonatomic, retain) NSMutableArray* collectionContactArray;
...
- (void)viewDidLoad {
    ...
    _addedAssignees = [NSArray array];
    _addedCCers = [NSArray array];
    ...
    _assigneesCell = [[CVLabelCell alloc] init];
//    [_assigneesCell setValue:[_updatedAssignees toDisplayString]];
    
    if ([_updatedAssignees count]>0)
        _assigneesCell.isAssigneeOrCc = YES;
    else
        _assigneesCell.isAssigneeOrCc = NO;
    
    // Image picker collection view
    
    if ([_updatedAssignees count] < 7) {
        _imageCollectionViewContainer = [[UIView alloc] initWithFrame:CGRectMake( 0, 0, 70* _updatedAssignees.count + 25, 100)];
    } else {
        _imageCollectionViewContainer = [[UIView alloc] initWithFrame:CGRectMake( 0, 0, 470, 100)];
    }
//    _imageCollectionViewContainer.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
    ...
    [flowLayout setMinimumInteritemSpacing:50.f];
    [flowLayout setMinimumLineSpacing:20.f];
    
    _imageCollectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, -15, _imageCollectionViewContainer.width ,  _imageCollectionViewContainer.height ) collectionViewLayout:flowLayout];
    _imageCollectionView.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
    
    [_imageCollectionView registerClass:[CVContactPickerImageCollectionWithNameCell class] forCellWithReuseIdentifier:@"selectingCollectionViewCell"];
...
//    _imageCollectionViewContainer.backgroundColor = [UIColor greenColor];
    [_assigneesCell addSubview:_imageCollectionViewContainer];
...
//    _collectionContactItem = [[CVUserListItem alloc] init];
//    _collectionContactArray = [NSMutableArray array];
//    [_collectionContactArray addObject:@"test For Assignees"];
//    for (NSDictionary* temp in _updatedAssignees){
//        _collectionContactItem = [CVUserListItem userListItemWithDictionary:temp];
//        [_collectionContactArray addObject:_collectionContactItem];
//    }
...
//    _assigneesCell.frame = CGRectMake(0, 0, [_updatedAssignees count] * 80 + 25, stampSize.height + 10*2);
//
////    _assigneesCell.stampsView.backgroundColor = [UIColor blueColor];
//    _assigneesCell.insects = UIEdgeInsetsMake(10, 6, 10, 0);
//    _assigneesCell.items = _collectionContactArray;
//    _assigneesCell.stampSize = stampSize;
//    _assigneesCell.stampSpacing = 10;
//    _assigneesCell.delegate = self;
//    if ([_updatedAssignees count] > 5)
//        _assigneesCell.stampsView.frame = CGRectMake(_assigneesCell.insects.left, _assigneesCell.insects.right, 6 * 75 + 22, stampSize.height + 10*2);
//    else
//        _assigneesCell.stampsView.frame = CGRectMake(_assigneesCell.insects.left, _assigneesCell.insects.right, ([_updatedAssignees count] + 1)* 75 + 22, stampSize.height + 10*2);
...
//    [_collectionContactArrayForCCers addObject:@"test For CCers"];
//
//    
...
//    _ccersCell.frame = CGRectMake(0, 0, [_updatedAssignees count] * 40 + 25, stampSize.height + 10*2);
////    _ccersCell.stampsView.backgroundColor = [UIColor greenColor];
//    _ccersCell.insects = UIEdgeInsetsMake(10, 6, 10, 0);
...
//    if ([_updatedCCers count]>6)
//        _ccersCell.stampsView.frame = CGRectMake(_ccersCell.insects.left, _ccersCell.insects.right, 6 * 75 + 22, stampSize.height + 10*2);
//    else
//        _ccersCell.stampsView.frame = CGRectMake(_ccersCell.insects.left, _ccersCell.insects.right, [_updatedCCers count] * 75 + 22, stampSize.height + 10*2);
//    [_ccersCell registerStampClass:[CVContactStampCell class] forCellWithReuseIdentifier:@"contact"];
    ...
    _ccersCell = [[CVLabelCell alloc] init];
//    [_ccersCell setValue:[_updatedCCers toDisplayString]];
    
    
    if ([_updatedAssignees count]>0)
        _ccersCell.isAssigneeOrCc = YES;
    else
        _ccersCell.isAssigneeOrCc = NO;
    
    // Image picker collection view
    
    if ([_updatedCCers count] < 7) {
        _imageCollectionViewContainerForCCers = [[UIView alloc] initWithFrame:CGRectMake( 0, 0, 70* _updatedCCers.count + 25, 100)];
    } else {
        _imageCollectionViewContainerForCCers = [[UIView alloc] initWithFrame:CGRectMake( 0, 0, 470, 100)];
    }
    
    UICollectionViewFlowLayout *flowLayout2 = [[UICollectionViewFlowLayout alloc] init];
    [flowLayout2 setScrollDirection:UICollectionViewScrollDirectionHorizontal];
    [flowLayout2 setItemSize:CGSizeMake(45, 45)];
    [flowLayout2 setMinimumInteritemSpacing:50.f];
    [flowLayout2 setMinimumLineSpacing:20.f];
    _imageCollectionViewForCCers = [[UICollectionView alloc] initWithFrame:CGRectMake(0, -15, _imageCollectionViewContainerForCCers.width ,  _imageCollectionViewContainerForCCers.height ) collectionViewLayout:flowLayout2];
    _imageCollectionViewForCCers.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
    
    [_imageCollectionViewForCCers registerClass:[CVContactPickerImageCollectionWithNameCell class] forCellWithReuseIdentifier:@"selectingCollectionViewCellForCCers"];
    _imageCollectionViewForCCers.delegate = self;
    _imageCollectionViewForCCers.dataSource = self;
    _imageCollectionViewForCCers.backgroundColor = [UIColor clearColor];
    _imageCollectionViewForCCers.showsHorizontalScrollIndicator = NO;
    [_imageCollectionViewContainerForCCers addSubview:_imageCollectionViewForCCers];
//    _imageCollectionViewContainerForCCers.backgroundColor = [UIColor blueColor];
    [_ccersCell addSubview:_imageCollectionViewContainerForCCers];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_selectedCell == _assigneesCell) {
        
        NSArray* initialAssignees = _addPeople ? _addedAssignees : _updatedAssignees;
        ...
        ((CVContactPickerViewController*)controller).placeHolder = LS(@"To:", @"");
    ...
    } else if (_selectedCell == _ccersCell) {
        NSArray* initialCCers = _addPeople ? _addedCCers : _updatedCCers;
        controller = [[CVContactPickerViewController alloc] initWithRecipients:initialCCers forAddPeople:_addPeople withOptions:@[TYPE_OPTION_ALL, TYPE_OPTION_TRUSTED, TYPE_OPTION_CONNECTED, TYPE_OPTION_ENGAGED, TYPE_OPTION_ACQUAINTANCE, TYPE_OPTION_GROUP, TYPE_OPTION_ADDBOOK]];
        ((CVContactPickerViewController*)controller).placeHolder = LS(@"Cc:", @"");
        ((CVContactPickerViewController*)controller).delegate = self;
        ((CVContactPickerViewController*)controller).listType = _isRestrictedTask ? CONTACT_LIST_TYPE_RESTRICTED_TASK : CONTACT_LIST_TYPE_TASK;
        
    } else if (_selectedCell == _dueDateCell || _selectedCell == _dueTimeCell) {
...
-(void)recipientsChanged:(NSArray*)recipients {
    ...
    if (_selectedCell == _assigneesCell && !_addPeople) {
        ...
        ///////////////
        ///////////////
//        [_collectionContactArray removeAllObjects];
//        [_collectionContactArray addObject:@"test"];
//        for (NSDictionary* temp in _updatedAssignees){
//            _collectionContactItem = [CVUserListItem userListItemWithDictionary:temp];
//            [_collectionContactArray addObject:_collectionContactItem];
//        }
...
//        CGSize stampSize = isIPAD() ? CGSizeMake(70, 105) : CGSizeMake(40, 40);
//        if ([_updatedAssignees count] > 5)
//            _assigneesCell.stampsView.frame = CGRectMake(_assigneesCell.insects.left, 0, 6 * 75 + 22, stampSize.height + 10*2);
//        else
//            _assigneesCell.stampsView.frame = CGRectMake(_assigneesCell.insects.left, 0, ([_updatedAssignees count] + 1) * 75 + 22, stampSize.height + 10*2);
//        
...
//        [_assigneesCell setValue:[_updatedAssignees toDisplayString]];
        if ([recipients count] < 7) {
            _imageCollectionViewContainer.frame = CGRectMake( 0, 0, 70* recipients.count + 25, 100);
        } else {
            _imageCollectionViewContainer.frame = CGRectMake( 0, 0, 470, 100);
        }
        
        if ([_updatedCCers count] < 7) {
            _imageCollectionViewContainerForCCers.frame = CGRectMake( 0, 0, 70* _updatedCCers.count + 25, 100);
        } else {
            _imageCollectionViewContainerForCCers.frame = CGRectMake( 0, 0, 470, 100);
        }

        [_imageCollectionView reloadData];
        [_imageCollectionViewForCCers reloadData];
...
//        CGSize stampSize = isIPAD() ? CGSizeMake(70, 105) : CGSizeMake(40, 40);
//        if ([_updatedCCers count]>6)
//            _ccersCell.stampsView.frame = CGRectMake(_ccersCell.insects.left, 0, 6 * 75 + 22, stampSize.height + 10*2);
//        else
//            _ccersCell.stampsView.frame = CGRectMake(_ccersCell.insects.left, 0, [_updatedCCers count] * 75 + 22, stampSize.height + 10*2);
//        
...
//        [_ccersCell setValue:[_updatedCCers toDisplayString]];
        ...
        if ([_updatedAssignees count] < 7) {
            _imageCollectionViewContainer.frame = CGRectMake( 0, 0, 70* _updatedAssignees.count + 25, 100);
        } else {
            _imageCollectionViewContainer.frame = CGRectMake( 0, 0, 470, 100);
        }
        ...
        _addedAssignees = recipients;
        NSMutableArray* combined = [NSMutableArray arrayWithArray:_updatedAssignees];
        [combined addObjectsFromArray:_addedAssignees];
        _updatedAssignees = combined;
//        [_assigneesCell setValue:[combined toDisplayString]];
    } else if (_selectedCell == _ccersCell && _addPeople && _addedCCers) {
        _addedCCers = recipients;
        NSMutableArray* combined = [NSMutableArray arrayWithArray:_updatedCCers];
        [combined addObjectsFromArray:_addedCCers];
        _updatedCCers = combined;
//        [_ccersCell setValue:[combined toDisplayString]];
    } else
        ;
    [self.tableView reloadData];
...
- (NSInteger)collectionView:(UICollectionView *)collectionView numberOfItemsInSection:(NSInteger)section {
    if (collectionView == self.imageCollectionView)
        return [_updatedAssignees count];
    else if (collectionView == self.imageCollectionViewForCCers)
        return [_updatedCCers count];
}

- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {

    if (collectionView == self.imageCollectionView) {
        CVContactPickerImageCollectionWithNameCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"selectingCollectionViewCell" forIndexPath:indexPath];
        // 	[cell.collectionImageView unsetImage];
        
        if (!cell)
            cell = [[CVContactPickerImageCollectionWithNameCell alloc] init];
        ...
        cell.displayName.text = [[collectionContactArray objectAtIndex:indexPath.item] displayName];
        return cell;

    } else {
        CVContactPickerImageCollectionWithNameCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"selectingCollectionViewCellForCCers" forIndexPath:indexPath];
        // 	[cell.collectionImageView unsetImage];
...
//        if ([((NSString*)object) isEqualToString:@"test For Assignees"]){// isEqualToString:@""])
//            initialAssignees = _addPeople ? _addedAssignees : _updatedAssignees;
//        } else if([((NSString*)object) isEqualToString:@"test For CCers"]){
